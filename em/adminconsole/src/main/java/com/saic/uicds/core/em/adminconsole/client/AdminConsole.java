 ...
 */
public class AdminConsole implements EntryPoint, UICDSConstants {
	...
	private Widget createServiceElementsTab() {
		...
		xsltPanel.setCellVerticalAlignment(xsltLabel,
				VerticalPanel.ALIGN_MIDDLE);
		xsltPanel
				.setCellVerticalAlignment(xsltList, VerticalPanel.ALIGN_MIDDLE);
		...
		formPanel.setCellHorizontalAlignment(xsltPanel,
				HorizontalPanel.ALIGN_RIGHT);
		formPanel.setCellHorizontalAlignment(moreInfo,
				HorizontalPanel.ALIGN_RIGHT);
	...
	private void createServicePanel() {
		...
		tree.getElement().setAttribute("tabIndex", "0");
		uicdsServicePanel.setLeftWidget(tree);
		uicdsServicePanel.getLeftWidget().getElement().setAttribute("tabIndex", "0");
		uicdsServicePanel.setRightWidget(createServiceElementsTab());
		uicdsServicePanel.setSplitPosition("35%");
		uicdsServicePanel.setSize("100%", (Window.getClientHeight() - 50)
				+ "px");
		com.google.gwt.user.client.Element elem = uicdsServicePanel
				.getElement();
		if (elem != null) {
			if (elem.getFirstChild() != null)
				if (elem.getFirstChild().getChildCount() > 0) {
					...
					if (child != null) {
						NodeList<Element> imgs = ((Element) child)
	...
	public void onModuleLoad() {
		...
		//rootTabPanel.getElement().setAttribute("tabIndex", "0");
	...
	private void setAriaForIFrames() {

		//Window.alert("table:" + RootPanel.get("__gwt_historyFrame").getElement().getId());
		Element frame1 = RootPanel.get("__gwt_historyFrame").getElement();
		frame1.setTitle("cacheFrame");
		frame1.setPropertyString("name", "uicds cache Frame");


		Element frame2 = RootPanel.get("com.saic.uicds.core.em.adminconsole.AdminConsole").getElement();
		frame2.setTitle("adminConsoleFrame");
		frame2.setAttribute("name", "uicds admin console frame");
		//Window.alert("table:" + frame2.getAttribute("name"));

	}

	private void setAriaForRootTab() {
		rootTabBar = rootTabPanel.getTabBar();
		rootTabNodes = rootTabBar.getElement().getFirstChild().getFirstChild()
				.getChildNodes();
		...
		Element uicdsInputElem = (Element) uicdsTabElem.getFirstChild()
				.getChild(1).getChild(1).getFirstChild().getFirstChild()
				.getFirstChild();
		uicdsInputElem.setTitle(UICDS);
		Element hnsInputElem = (Element) hnsTabElem.getFirstChild().getChild(1)
				.getChild(1).getFirstChild().getFirstChild().getFirstChild();
	...
	private void setAriaForContentsTab() {
		...
		contentsTabNodes = contentsTabBar.getElement().getFirstChild()
		...
		Element formInputElem = (Element) formTabElem.getFirstChild()
				.getChild(1).getChild(1).getFirstChild().getFirstChild()
				.getFirstChild();
		formInputElem.setTitle(FormView);
		Element xmlInputElem = (Element) xmlTabElem.getFirstChild().getChild(1)
				.getChild(1).getFirstChild().getFirstChild().getFirstChild();
