function AgreementController($scope, $http) {
  ...
  $scope.init = function() {
	...
	$scope.addShareRule = function(agreement) {
		...
		agreement.ShareRules[0].ShareRule.push(
			{"attr": {
             ...
             },"Condition": [	{"InterestGroup": [{ "attr": {"label" : {"value":"All Incident Types"}, "code" : {"value":"*"}}}],"RemoteCoreProximity": [],	"ExtendedMetadata": []}	]}
		);
	};
	...
	var xmlAgreementTmpl =
		...
		'<as:Principals>',
		'<as:LocalCore precisb:label="Local Core">{{{localCore}}}</as:LocalCore>',
		...
		'{{#ShareRules.0.ShareRule}}',
			...
			'{{#Condition}}',
				'<as:Condition>',
				'{{#InterestGroup}}',
					'<as:InterestGroup precisb:label="{{attr.label.value}}" precisb:codespace="{{{attr.codespace.value}}}">{{attr.code.value}}</as:InterestGroup>',
				'{{/InterestGroup}}',
				'{{#ExtendedMetadata}}',
