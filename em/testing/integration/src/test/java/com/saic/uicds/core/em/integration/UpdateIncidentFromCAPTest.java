...
public class UpdateIncidentFromCAPTest
    extends AbstractXmldbJpaTests {

    Logger log = LoggerFactory.getLogger(UpdateIncidentFromCAPTest.class);
    ...
    @Test
    public void testUpdateIncidentFromCAP() throws XmlException, IOException {

        /*
         * UICDSAlertAdapterType alertAdapter = UICDSAlertAdapterType.Factory.newInstance();
         */
        ...
        assertTrue("Activity name not correct expecting: " + alertEvent + " got "
            + incident.getIncident().getActivityNameArray(0).getStringValue(), matcher.find());
        ...
        incident.getIncident().getActivityCategoryTextArray(0).setStringValue("CBRNE");

        status = incidentManagementService.updateIncident(incident.getIncident(),
            WorkProductHelper.getWorkProductIdentification(wp));
        wp = workProductService.getProduct(status.getProduct().getProductID());

        IncidentDocument updatedIncident = (IncidentDocument) wp.getProduct();

        assertFalse(updatedIncident.isNil());
        logger.debug("updatedIncident:\n\n[" + updatedIncident + "]\n\n");

        pattern = Pattern.compile("<[a-z0-9]+:ActivityCategoryText>CBRNE</[a-z0-9]+:ActivityCategoryText>");
        matcher = pattern.matcher(updatedIncident.toString());
        // TODO assertTrue(matcher.find());
