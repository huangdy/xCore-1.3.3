            ...
            for (com.saic.uicds.core.infrastructure.model.ShareRule rule : agreement.getShareRules()) {
                ...
                if (rule.getExtendedMetadata() != null && rule.getExtendedMetadata().size() > 0) {
                    ...
                    for (ExtendedMetadata em : rule.getExtendedMetadata() ) {
                        ...
                        log.debug("copying extended metadata: " + extendedMetadata.getCodespace() + " - " + extendedMetadata.getCode() + " = " + extendedMetadata.getStringValue());
                    }
                    
                }
                
                if (rule.getRemoteCoreProximity() != null  ) {
                    log.debug("copying Proximity value: " + rule.getRemoteCoreProximity().toString());
