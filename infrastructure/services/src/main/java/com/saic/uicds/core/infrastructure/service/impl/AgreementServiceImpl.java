        ...
        if (agreementType.getShareRules() != null
            ...
            for (AgreementType.ShareRules.ShareRule shareRule : agreementType.getShareRules().getShareRuleArray()) {
                ...
                if (shareRule.getCondition() == null) {
                    ...
                    log.debug("checking for extendedMetadata");
                    ...
                    if (shareRule.getCondition().getExtendedMetadataArray() != null &&
                        ...
                        log.debug("Added extendedmetadata to the rule: size=" + rule.getExtendedMetadata().size());
                    } else {
                        log.debug("no extended metadata in share rule");
                    }
                    
                    if (shareRule.getCondition().getRemoteCoreProximity() != null) {
                        log.debug("Share Rule has remote core proximity " + shareRule.getCondition().getRemoteCoreProximity().getStringValue());
                        log.debug("Share Rule has share on no location - " + shareRule.getCondition().getRemoteCoreProximity().getShareOnNoLoc());
                        ...
                        log.debug("no remote core proximity in share rule");
                    }
