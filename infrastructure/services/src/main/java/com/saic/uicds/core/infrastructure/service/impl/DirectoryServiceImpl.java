...
public class DirectoryServiceImpl implements DirectoryService {
    ...
    // Key: Core Name
    // Value: "lat lon"
    private HashMap<String, String> coreLocationMap = new HashMap<String, String>();
    ...
    private synchronized void addCoreToStatusMap(String coreName, Enum updatedStatus, String lat, String lon) {
        String location = lat + " " + lon;
        log.debug("Adding core, " + coreName + ", to status map.  Location: " + location);
        coreStatusMap.put(coreName, updatedStatus);
        
        coreLocationMap.put(coreName.toLowerCase(), location);
        log.debug("Added " + coreName + " to status map - retrieved location: " + coreLocationMap.get(coreName));
    }
    ...
    public synchronized CoreConfigListType getCoreList() {
        ...
        for (String coreName : coreNames) {
            log.debug("getCoreList: " + coreName);
            ...
            if (coreLocationMap.containsKey(coreNameLower) ) {
                log.debug("corename: " + coreNameLower);
                log.debug("\tlocString: " + coreLocationMap.get(coreNameLower));

                String[] location = coreLocationMap.get(coreNameLower).split("\\s");
                log.debug("\tlocation.length: " + location.length);
                log.debug("\tlocation[]: " + location[0] + " " + location[1]);
    ...
    public synchronized CoreConfigType getCoreConfig(String coreName) {
        ...
        log.debug("getCoreConfig: " + coreName);
        ...
        if (coreName.endsWith("/CoreConnection")) {
            ...
            if (coreStatus.equals("unsubscribed")) {
                ...
                if (! ("".equals(message.getCoreLatitude()) || "".equals(message.getCoreLongitude())))  {
                     addCoreToStatusMap(coreName, updatedStatus, message.getCoreLatitude(), message.getCoreLongitude());
                } else {
                    addCoreToStatusMap(coreName, updatedStatus);
                }
            }
