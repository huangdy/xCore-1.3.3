...
public class RosterManager {
    ...
    class CoreRosterListener
        ...
        public void presenceChanged(Presence presence) {
            ...
            if (type == Presence.Type.available || type == Presence.Type.unavailable) {
                DefaultPacketExtension geoloc = (DefaultPacketExtension) presence.getExtension("geoloc", "http://jabber.org/protocol/geoloc");
                if (geoloc == null) {
                    sendCoreStatusUpdate(fullJID, type.toString(), "", "");
                } else {
                    sendCoreStatusUpdate(fullJID, type.toString(), geoloc.getValue("lat"), geoloc.getValue("lon"));
                }
            }
        ...
        if (!bareJID.equalsIgnoreCase(connection.getJID())) {
            ...
            if (rosterMap.containsKey(bareJID.toLowerCase())) {
                ...
                if (geoloc == null) {
                    ...
                    sendCoreStatusUpdate(fullJID, presence.getType().toString(), geoloc.getValue("lat"), geoloc.getValue("lon"));
                }
            } else {
    ...
    public void sendCoreStatusUpdate(String fullJID, String coreStatus, String latitude, String longitude) {
        ...
        if (coreName != null) {
            logger.info("RosterManager:sendCoreStatusUpdate - sending core status update: ["
                + fullJID + "," + coreStatus + "]");
            CoreStatusUpdateMessage msg = new CoreStatusUpdateMessage(fullJID, coreStatus, latitude, longitude);
