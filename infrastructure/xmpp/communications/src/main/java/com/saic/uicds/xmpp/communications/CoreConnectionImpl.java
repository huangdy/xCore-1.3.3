    ...
    public void connect() {
        ...
        if (!connected) {
            try {
                ...
                //String configFile = System.getProperty("user.dir") + p + "ServerApps" + p + "uicds" + p + "config" + p + "resources" + p + "config.xml";

                DocumentBuilderFactory factory = DocumentBuilderFactory.newInstance();
                factory.setNamespaceAware(false);
                DocumentBuilder builder;
                Document doc = null;
                XPathExpression posExpr = null;
                builder = factory.newDocumentBuilder();
                doc = builder.parse(coreConfigFile);
                ...
                // only take the first geomettric point
                posExpr = xpath.compile("//GeoLocation/Point/Point/pos/text()");
                // Run the query and get a nodeset
                logger.debug("got this far...");
                Object result = posExpr.evaluate(doc, XPathConstants.NODESET);
                NodeList nodes = (NodeList) result;
                logger.debug("nodes length: " + nodes.getLength());
                if (nodes.getLength() > 0) {
                    
                    String[] pos = nodes.item(0).getNodeValue().split(" ");
                    logger.debug("adding geo to presence: " + nodes.item(0).getNodeValue());
                    // add packet interceptor
                    PacketFilter presenceFilter = new PacketTypeFilter(Presence.class);
                    PresenceEnrichment presenceEnrichment = new PresenceEnrichment(pos[0], pos[1]);
                    xmppConnection.addPacketInterceptor(presenceEnrichment, presenceFilter);
                } else {
                    logger.debug("Config file, " + coreConfigFile + ", contained no valid gemoetry.");
                }
